...
class GzhiHandler
{
    ...
    public function handleGzhiTicket ( Ticket $ticket, GzhiApiProvider $gzhiProvider ) : int
    {
        ...
        if ( ! isset( $ticket->managements[ 0 ]->management->guid ) || ! $ticket->type->gzhi_code_type || ! $ticket->type->gzhi_code || $ticket->building->gzhi_address_guid == null || $ticket->vendors()
                ->where( [ 'vendor_id' => GzhiRequest::GZHI_VENDOR_ID ] || $ticket->type_id == null || $ticket->status_code == 'draft' || $ticket->type->gzhi_code == '36006' || $ticket->type->gzhi_code_type == '36' || !in_array($ticket->status_code, GzhiRequest::GZHI_STATUSES_LIST) )
                ->count() )
        {
            return 0;
        }
